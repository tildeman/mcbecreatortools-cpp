...
template <typename jso> json_object<jso>::json_object(vector<pair<string,jso>> a){
	this->object_pair=a;
}

template <typename jso> jso& json_object<jso>::operator[](string s){
...
string json_value::indent_json(string s){
	string o="";
...
string json_value::strip(string s){
	int begin,end;
...
int json_value::hex_to_int(string s){
	int l=s.length(),i;
	int c=-1,r=0;
...
string json_value::unescapify(string s){
	int i,l=s.length();
	string r;
	for (i=0;i<l;i++){
		int p1=0,p2=0;
...
json_value::json_value(string s){
	val_type=1;
	val_string=s;
}
json_value::json_value(vector<json_value> l){
	val_type=2;
	val_list=l;
}
...
*/
json_value json_value::parse(string s){
