...
const uuid_generator_class = class {
	constructor(options) {
		...
		this.data.only_heksadecimal_characters = !options.only_numbers 
			? (options.only_heksadecimal_characters || false) 
			: false;
		this.data.use_uppercase_letters = !options.only_numbers 
			? (options.use_uppercase_letters || false) 
			: false;
	...
	generate_uuid() {
		...
		if (this.data.only_numbers) {
			for (let index = 0; index < uuid_version_pattern[this.data.uuid_version].length; index++) {
				for (let counter = 1; counter <= uuid_version_pattern[this.data.uuid_version][index]; counter++) 
					generated_uuid += String(uuid_decimal[Math.floor(Math.random() * uuid_decimal.length)]);
			...
			for (let index = 0; index < uuid_version_pattern[this.data.uuid_version].length; index++) {
				for (let counter = 1; counter <= uuid_version_pattern[this.data.uuid_version][index]; counter++) 
					generated_uuid += String(uuid_heksadecimal_characters[Math.floor(Math.random() * uuid_heksadecimal_characters.length)]);
				if (index < uuid_version_pattern[this.data.uuid_version].length - 1) generated_uuid += String("-");
			};
		} else {
			for (let index = 0; index < uuid_version_pattern[this.data.uuid_version].length; index++) {
				for (let counter = 1; counter <= uuid_version_pattern[this.data.uuid_version][index]; counter++) 
					generated_uuid += String(uuid[Math.floor(Math.random() * uuid.length)]);
				if (index < uuid_version_pattern[this.data.uuid_version].length - 1) generated_uuid += String("-");
			};
		};
