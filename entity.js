...
genButton.onclick = function () {
	if (is_valid_identifier(identifier.value)) {
		...
		isSpawnable.checked && (
			d["minecraft:client_entity"].description.spawn_egg = {}, 
			d["minecraft:client_entity"].description.spawn_egg.overlay_color = spawnEggOverlay.value, 
		...
		d["minecraft:client_entity"].description.materials["default"] = material.value;
		0 < modelFile.files.length && (
			d["minecraft:client_entity"].description.geometry = {}, 
			"res" !== select.value && "both" !== select.value || b.file("resource_pack/models/entity/" + modelFile.files[0].name, 
			modelFile.files[0]), 
			d["minecraft:client_entity"].description.geometry["default"] = geoID.toLowerCase()
		);
...
function genBehBasic(a) {
	...
	a["minecraft:entity"].components["minecraft:health"].value = 20;
...
function genBehCustom(a) {
	...
	health.value && !isImmune.checked && (
		a["minecraft:entity"].components["minecraft:health"] = {}, 
		a["minecraft:entity"].components["minecraft:health"].value = parseFloat(health.value)
	);
	scale.value && (
		a["minecraft:entity"].components["minecraft:scale"] = {}, 
		a["minecraft:entity"].components["minecraft:scale"].value = parseFloat(scale.value)
	);
	typeFamily.value && (
		a["minecraft:entity"].components["minecraft:type_family"] = {}, 
		a["minecraft:entity"].components["minecraft:type_family"].family = [], 
		a["minecraft:entity"].components["minecraft:type_family"].family = typeFamily.value.split(",")
	);
	isImmune.checked && (
		a["minecraft:entity"].components["minecraft:damage_sensor"] = {}, 
		a["minecraft:entity"].components["minecraft:damage_sensor"].triggers = {}, 
		a["minecraft:entity"].components["minecraft:damage_sensor"].triggers.cause = "all", 
		a["minecraft:entity"].components["minecraft:damage_sensor"].triggers.deals_damage = "false");
	collisionH.value && collisionW.value && (
		a["minecraft:entity"].components["minecraft:collision_box"] = {}, 
		a["minecraft:entity"].components["minecraft:collision_box"].height = parseFloat(collisionH.value), 
		a["minecraft:entity"].components["minecraft:collision_box"].width = parseFloat(collisionW.value)
	)
}
...
function genManifest(a) {
	...
	b.modules[0].uuid = a.generate_uuid();
	...
	"both" === select.value && (
		...
		c.dependencies[0].version = b.header.version
	);
	return {
		rp: c,
		bp: b
